...
2. 데이터를 통한 공식화된 자료: 이게임 알아? 응 마니 알아 점심시간에 다들 오 그개임 만들어”? 라는 호평을 얻음 이로인해서 이 게임을 개발하면 즐겨찾는 이가 많을 거라는 결로누 → 만듦

3. 타사 제품 비교~기존의 무엇은 뭔데 우리는 이렇다 아니면 이렇게 만드려 했다  1
...
7. 기술스택 : html css js git 잔디 vsc GPT3.5
...
- 근데 나도 만들고 싶다 해서 한명이서 하나의 맵을 개발하기로 함
...
- 지금 게임내부디자인은 모두 잡혔으니까 네가지의 게임 기능을 네 명이서 나누어서 하면 더 효울적으로 할 수 있을거라고 생각함 어띃게 생각하세요??
...
구현기능 병합 - app.js : 다같이 함

배너, 데스카운팅 : 한기범

코인 다머겅야 넘어가게 하기 : 다같이 찾다가 황정원

윤종 : 다같이 하는거에 어중떠중 하다가 누가 자꾸 해결해서 중도에 끝냄

이럴거면 그냥 기능 하나를 한명이서 잡고 개발하는게 나한테는 더 맞는듯

혼자 빠르게 집중해서 끝낼수 있는 전문성 기르기!

팀플 할 때 다같이 하는 건 우선순위가 낮은 걸로, 우선순위가 높은  기능은 혼자 개발하는 것으로 의견제시하기

- 구현기능 병합, 부가기능 개발
1. 본인 새로운 맵 디자인하기 : 다같이
2. 캐릭터 움직이는 모션 부드럽게 : 다같이
3. 시연할 때 못깰경우 숨겨진 클리어존 만들기, 카운트 : 기범
...
- js 게임로직구현 황준원 김병모 한기범
- 메인페이지, 진입, 헤더푸터, 게임 구현 후 합치기
- 맵 850 x 600
- 맵 구현 → html, css 4월 7일 일요일까지
